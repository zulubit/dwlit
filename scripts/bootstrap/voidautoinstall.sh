...
sudo xbps-install -y NetworkManager Thunar tumbler gthumb meson base-devel base-system bluetuith curl mako elogind flatpak fcft polkit polkit-elogind lxsession void-repo-nonfree fcft-devel swaylock firefox foot gcc git libxml2-devel xdg-desktop-portal-wlr xdg-desktop-portal-gtk brightnessctl make mesa-dri pavucontrol pipewire qt5-wayland qt6-wayland sof-firmware sof-tools swaybg wget wl-clipboard wlroots-devel wlroots0.18 wlroots0.18-devel rofi dejavu-fonts-ttf xorg-server-xwayland grim slurp swappy vlc wf-recorder || error_exit "Package installation failed."
